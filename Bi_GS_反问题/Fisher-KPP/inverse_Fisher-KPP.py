# 2D Fisher-KPP 方程 Bi-GS-PINN 实验代码
import os
import time
import numpy as np
import torch
...
class Config:
    ...
    nu_fisher = 0.05        # 扩散系数
    rho = 20.0               # 反应速率
    # k = sqrt(rho/(6*nu)) ≈ 0.408, omega = 5*rho/6 ≈ 0.833, c ≈ 2.04
    ...
    RUN_STRATEGY = "bi_gs"  # 可选 "all", "standard", "pcgrad", "bi_gs", "moo_vari", "gradnorm"
    ...
    gn_initial_losses_decay = 1.0  # 初始损失 EMA 衰减
...
# ==============================================
# 📌 2D Fisher-KPP 解析解（平面行波，45°方向）
#    u(z,t) = [1 + exp(k(z - ct))]⁻²
#    其中 z = (x+y)/√2, k = √(ρ/(6ν)), c = 5√(νρ/6)
#    PDE 残差经验证 ≈ 1e-16（机器精度）
# ==============================================
...
def exact_solution_fisher(X_np):
    """
    二维 Fisher-KPP 平面行波解析解。
    X_np: numpy array shape (N, 3)，列顺序 (x, y, t)
    返回: numpy array shape (N, 1)
    """
    x = X_np[:, 0:1]
    y = X_np[:, 1:2]
    t = X_np[:, 2:3]
    z = (x + y) / np.sqrt(2.0)            # 旋转坐标
    xi = k_fisher * (z - c_fisher * t)     # 行波变量
...
# 生成参考解（均匀网格，用于评价）
x_ref = np.linspace(Config.x_min, Config.x_max, 65)   # 65 点，步长 1/32
y_ref = np.linspace(Config.y_min, Config.y_max, 65)
t_ref = np.linspace(Config.t_min, Config.t_max, 21)   # 21 snapshots
...
# ==============================================
# 📉 PDE 残差：u_t - ν(u_xx+u_yy) - ρ·u·(1-u) = 0
# ==============================================
def pde_loss(net, X):
    u = net(X)
...
def apply_bigspinn_surgery(losses, weights, net, gamma=0.5):
    ...
    for w, loss in zip(weights, losses):
        g = torch.autograd.grad(w * loss, net.parameters(), retain_graph=True, create_graph=False, allow_unused=True)
        flat_grad = torch.cat([(grad if grad is not None else torch.zeros(1, device=Config.device)).view(-1) for grad in g])
...
def apply_pcgrad(losses, net):
    ...
    grads = []
    for loss in losses:
        g = torch.autograd.grad(loss, net.parameters(), retain_graph=True, create_graph=False, allow_unused=True)
        flat_grad = torch.cat([(grad if grad is not None else torch.zeros(1, device=Config.device)).view(-1) for grad in g])
        grads.append(flat_grad)
...
def get_flat_params(net, inverse=False):
    """将网络参数展平为一维 numpy 数组。反问题时包含 rho_param。"""
    params_list = []
...
def set_flat_params(net, flat, inverse=False):
    """将一维 numpy 数组写回网络参数。返回参数总数（不含 rho_param）。"""
    start = 0
    for p in net.parameters():
        numel = p.numel()
...
def evaluate_fitness(net, flat, inverse, X_f, X_bc, X_ic, X_data=None, u_data=None):
    """对一个个体评估全部损失项。返回 numpy 数组 [loss_pde, loss_bc, loss_ic, (loss_data)]。"""
    set_flat_params(net, flat, inverse)
    # Fisher-KPP PDE 残差（需要 grad）
...
def main(strategy="standard", inverse=False, seed=0, timestamp=""):
    ...
    # For inverse problem: generate observation data
    if inverse:
        N_data = 200
        X_data_np = np.random.uniform(-1, 1, (N_data, 3))
        X_data_np[:, 2] *= Config.t_max  # t ∈ [0, T]
        X_data = torch.tensor(X_data_np, dtype=torch.float32).to(Config.device)
        u_data = torch.tensor(exact_solution_fisher(X_data_np), dtype=torch.float32).to(Config.device)
    ...
    num_losses = 4 if inverse else 3  # + data loss
    ...
    for epoch in range(Config.adam_epochs):
        ...
        if strategy == "standard":
            ...
            # MOO-VARI: 每 moo_freq 步用 NSGA-II+VARI 更新权重
        ...
        if current_iter % Config.eval_step == 0:
            ...
            if current_iter % Config.print_step == 0:
                rho_str = f"| rho={net.rho_param.item():.4f}" if inverse else ""
                print(f"Adam Epoch {epoch:5d} | Loss={loss.item():.2e} | L2={l2:.3e} {rho_str}")
    ...
    if Config.lbfgs_max_iter > 0:
        ...
        def closure():
            ...
            losses_lbfgs = [loss_pde_l, loss_bc_l, loss_ic_l]
            if inverse:
                u_data_pred_l = net(X_data)
                loss_data_l = torch.mean((u_data_pred_l - u_data)**2)
                losses_lbfgs.append(loss_data_l)
            loss_l = sum(w * l for w, l in zip(final_weights, losses_lbfgs))
            loss_l.backward()
            ...
            if lbfgs_iter_count[0] % 500 == 0 or lbfgs_iter_count[0] == 1:
                ...
                rho_str_l = f"| rho={net.rho_param.item():.4f}" if inverse else ""
                print(f"L-BFGS Iter {lbfgs_iter_count[0]:5d} | Loss={loss_l.item():.2e} | L2={l2_l:.3e} {rho_str_l}")
    ...
    with open(os.path.join(current_save_dir, "config.txt"), "w") as f:
        ...
        if inverse:
            ...
            f.write(f"rho_est = {net.rho_param.item():.6f}\n")
        f.write(f"\n--- Final L2 Error ---\n")
        f.write(f"L2 = {l2_final:.6e}\n")

    print(f"Plots saved to {current_save_dir}")
    # 反问题：保存最终参数估计
    rho_est = net.rho_param.item() if inverse else None
    if inverse:
        print(f"\n[Table 3: Parameter Estimation]")
        print(f"  --> rho (estimated): {rho_est:.4f} (true: {Config.rho})")

    torch.save(net.state_dict(), os.path.join(current_save_dir, "model_final.pth"))

    return l2_final, iter_to_1e3, rho_est
...
if __name__ == "__main__":
    ...
    for strategy in strategies_to_test:
        ...
        for s in range(max(5, num_runs) if not Config.DEBUG_MODE else num_runs):
            ...
            l2_final, iter_to_1e3, rho_est = main(strategy=strategy, inverse=inverse_mode, seed=s, timestamp=run_timestamp)
            l2_list.append(l2_final)
            if iter_to_1e3 != -1:
                iter_list.append(iter_to_1e3)
            if inverse_mode and rho_est is not None:
        ...
        results[key] = {
            ...
            "rho_mean": np.mean(rho_list) if inverse_mode else None,
            "rho_std": np.std(rho_list) if inverse_mode else None,
        }
    ...
    print("=" * 80)
    if inverse_mode:
        print("| Method              | Mode    | L2 Error (x 10^-3) | Iter to 1e-3 | Est. rho (true: {:.1f}) |".format(Config.rho))
        print("|---------------------|---------|--------------------|--------------|---------------------------|")
    else:
    ...
    for strategy in strategies_to_test:
        ...
        if inverse_mode:
            ...
            print(f"| {name_str:<20} | {mode_str_table:<7} | {l2_str:<18} | {iter_str:<12} | {rho_str:<25} |")
        else:
