...
print("=" * 70)
print(f"3D+time MMS loss / L2 comparison (Seed {SEED})")
print("=" * 70)
...
def draw_combined_figure():
    ...
    pdf_path = os.path.join(
        SAVE_DIR,
        f"3DMMS_seed{SEED}_loss_l2_compare.pdf"
    )

    png_path = os.path.join(
        SAVE_DIR,
        f"3DMMS_seed{SEED}_loss_l2_compare.png"
    )
...
draw_single_figure(
    y_key="loss",
    y_label="Loss",
    out_name=f"3DMMS_seed{SEED}_loss_compare"
)

draw_single_figure(
    y_key="l2",
    y_label="L2 Relative Error",
    out_name=f"3DMMS_seed{SEED}_l2_compare"
)
